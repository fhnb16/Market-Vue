...
function storyFit(storyItem) {
  if (
    storyItem.storyMediaFit == undefined ||
    storyItem.storyMediaFit == "" ||
    storyItem.storyMediaFit == "cover"
  ) {
    return "cover";
  } else {
    return "contain; min-height:unset";
...
/*const Paused = function (e, indx, actve) {
  if (indx == actve) {
    if (stories.value[actve].storyMediaType == "video") {
      if (e.target.paused) {
        e.target.play();
      }
    }
...
<template>
  ...
  >
    ...
    <div
      class="stories-item-content"
      v-for="(story, index) in stories"
      :key="'story-item-' + index"
      v-bind:class="index == activeStory ? 'itemPlaying' : ''"
    >
      <template
        v-if="
          story.storyMediaType == 'image' ||
          story.storyMediaType == undefined ||
          story.storyMediaType == ''
        "
      >
        <img
          v-bind:src="story.storyMedia"
          v-bind:style="'object-fit: ' + storyFit(story)"
          :rel="'story' + index"
        />
      </template>
      <template v-if="story.storyMediaType == 'video'">
        <video
          v-bind:src="story.storyMedia"
          v-bind:style="'object-fit: ' + storyFit(story)"
          playsinline
          webkit-playsinline
          v-bind:poster="
            story.storyMediaPoster != undefined ? story.storyMediaPoster : ''
          "
          :rel="'story' + index"
          autoplay
          loop
...
.stories-item-content.itemPlaying {
  opacity: 1;
  z-index: 8;
}
